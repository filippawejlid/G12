...
export function addToCart(clicked: number) {  
	...
	if (cartItems.length == 0) {
		...
		console.log("La till " + productInfo[clicked].productName +  productInfo[clicked].beenAdded);
		...
		console.log("La till ny " + productInfo[clicked].productName + productInfo[clicked].beenAdded);
	...
	for (let i = 0; i < cartItems.length; i++) {
		...
		console.log("Alla Objekt: " + cartItems[i].productName + " " + cartItems[i].amount);
	}
